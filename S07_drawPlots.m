...
function S07_drawPlots
  ...
  if ~exist('yo.mat','file')
    ...
    ticks.rez=300;
%      ticks.rez=42;
    ticks.width=297/25.4*ticks.rez/7;
...
function histstuff(vecs,DD,ticks)
    ...
    %%
    ratioBar(hc,'lat', range.(sen).lat,lainc);
    ...
    %%
    ratioBar(hc,'age', range.(sen).age,ageinc);
...
%     set(gca,'XTick',xt)
    ...
    set(gca,'yticklabel',sprintf('%3.0f|',xt*inc));
%     set(gca,'xticklabel',sprintf('%3.0f|',xlab));
end
