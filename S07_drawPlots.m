...
function taskfForZonMeans(DD,IN,ticks)
	pd={fullfile(pwd,'SUBS'),pwd};
	jm = findResource();
	veljob = createParallelJob(jm);
	scalejob = createParallelJob(jm);
	createTask(veljob, @velZonmeans, 0, {DD,IN,ticks})
	createTask(scalejob, @scaleZonmeans, 0, {DD,IN,ticks})
	set(veljob, 'PathDependencies',pd);
	set(scalejob, 'PathDependencies',pd);
	submit(veljob)
	submit(scalejob)
end
...
function taskfForMapAndHist(DD,IN,ticks)
	...
	createTask(hjob, @histstuff, 0, {IN.vecs,DD,ticks})
	createTask(mjob, @mapstuff, 0, {IN.maps,IN.vecs,DD,ticks,IN.lo,IN.la})
	set(hjob, 'PathDependencies',pd);
	set(mjob, 'PathDependencies',pd);
	submit(mjob)
	submit(hjob)
end
...
function trackPlots(DD,ticks,tracks)
	...
	for sense=senses; sen=sense{1};
		jm = findResource();
		ajob = createParallelJob(jm);
		bjob = createParallelJob(jm);
		cjob = createParallelJob(jm);
		djob = createParallelJob(jm);
		ejob = createParallelJob(jm);
		fjob = createParallelJob(jm);
		createTask(ajob, @TPa, 0, {DD,ticks,tracks,sen});
		createTask(bjob, @TPb, 0, {DD,ticks,tracks,sen});
		createTask(cjob, @TPc, 0, {DD,ticks,tracks,sen});
		createTask(djob, @TPd, 0, {DD,ticks,tracks,sen});
		createTask(ejob, @TPe, 0, {DD,ticks,tracks,sen});
		createTask(fjob, @TPf, 0, {DD,ticks,tracks,sen});
		submit(ajob)
		submit(bjob)
		submit(cjob)
		submit(djob)
		submit(ejob)
		submit(fjob)
		set(ajob, 'PathDependencies',pd);
		set(bjob, 'PathDependencies',pd);
		set(cjob, 'PathDependencies',pd);
		set(djob, 'PathDependencies',pd);
		set(ejob, 'PathDependencies',pd);
		set(fjob, 'PathDependencies',pd);
	end
end
