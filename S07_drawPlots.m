...
function S07_drawPlots
	...
	%%	set ticks here!
	ticks.rez=300;
	ticks.width=297/25.4*ticks.rez*sqrt(2)*4;
	ticks.height=ticks.width/4;
...
function scaleZonmeans(DD,IN,ticks)
	plot(IN.la(:,1),IN.maps.zonMean.AntiCycs.radius.mean.mean,'r'); 	hold on
	plot(IN.la(:,1),IN.maps.zonMean.Cycs.radius.mean.mean,'black')
	set(gca,'xtick',ticks.x	)
	set(gca,'ytick',ticks.y	)
	if DD.switchs.RossbyStuff
		plot(IN.la(:,1),2*IN.maps.zonMean.Rossby.small.radius)
		legend('Rossby Radius','anti-cyclones radius','cyclones radius')
	else
		legend('anti-cyclones radius','cyclones radius')
	end
	ylabel('[m]')
	xlabel('[latitude]')
